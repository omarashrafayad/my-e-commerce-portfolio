...
const cartslice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(GetUserCart.pending, (state) => {
            ...
            .addCase(GetUserCart.fulfilled, (state, action: PayloadAction<Products[]>) => {
                state.cart = action.payload
                state.loading = false
            }).addCase(GetUserCart.rejected, (state) => {
                state.loading = false
            })
            .addCase(AddToCart.fulfilled, (state, action: PayloadAction<Products[]>) => {
                state.cart = action.payload
            })
            .addCase(DeleteCart.fulfilled, (state, action: PayloadAction<string>) => {
                state.cart = state.cart.filter((f) => f.productId._id !== action.payload)
            }).addCase(
                IncreasQuantity.fulfilled,
                (state, action: PayloadAction<{ quantity: number; productId: string }>) => {
            ...
            ).addCase(decreaseQuantity.fulfilled, (state, action: PayloadAction<{ quantity: number; productId: string }>) => {
                const exist = state.cart.find(
                    (f) => f.productId._id === action.payload.productId
                );
                if (exist) {
